...
namespace ReGoap.Unity.FSMExample.Actions
{
    public class GatherResourceAction : ReGoapAction<string, object>
    {
            ...
            GoapActionStackData<string, object> stackData)
        {
            ...
            if (stackData.settings.HasKey("resource"))
            {
                effects.Set("hasResource" +
                            ((IResource)stackData.settings.Get("resource")).GetName(), true);
            }
            return effects;
        }
            ...
            GoapActionStackData<string, object> stackData)
        {
            ...
            if (newNeededResourceName != null &&
                stackData.currentState.HasKey("resource" + newNeededResourceName))
            {
                var results = new List<ReGoapState<string, object>>();
                Sensors.ResourcePair best = new Sensors.ResourcePair();
                var bestScore = float.MaxValue;
                foreach (var wantedResource in
                    (List<Sensors.ResourcePair>)stackData.currentState.Get("resource" + newNeededResourceName))
                {
