    ...
    public class ReGoapAgent<T, W> : MonoBehaviour, IReGoapAgent<T, W>, IReGoapAgentHelper
    {
        ...
        protected virtual bool CalculateNewGoal(bool forceStart = false)
        {
            ...
            currentReGoapPlanWorker = ReGoapPlannerManager<T, W>.Instance.Plan(
                this, BlackListGoalOnFailure ? currentGoal : null,
        ...
        protected virtual void OnDonePlanning(IReGoapGoal<T, W> newGoal)
        {
            ...
            {
                for (var i = 0; i < startingPlan.Count; i++)
                {
                    startingPlan[i].Action.PlanExit(
                        i > 0 ? startingPlan[i - 1].Action : null,
            ...
            for (var i = 0; i < startingPlan.Count; i++)
            {
                startingPlan[i].Action.PlanEnter(
                    i > 0 ? startingPlan[i - 1].Action : null,
                    i + 1 < startingPlan.Count ? startingPlan[i + 1].Action : null,
                    startingPlan[i].Settings, currentGoal.GetGoalState());
            }
        ...
        protected virtual void PushAction()
        {
            ...
            else
            {
                ...
                currentActionState.Action.Run(previous?.Action, next, currentActionState.Settings,
                    currentGoal.GetGoalState(), WarnActionEnd, WarnActionFailure);
            }
        }
        ...
        public virtual void WarnActionFailure(IReGoapAction<T, W> thisAction)
        {
            if (currentActionState != null && thisAction != currentActionState.Action)
            {
                ReGoapLogger.LogWarning(
                    $"[GoapAgent] Action {thisAction} warned for failure but is not current action.");
                return;
            }
        ...
        public virtual void WarnGoalEnd(IReGoapGoal<T, W> goal)
        {
            ...
            {
                ReGoapLogger.LogWarning(
                    $"[GoapAgent] Goal {goal} warned for end but is not current goal.");
                return;
            }
            CalculateNewGoal();
        }
