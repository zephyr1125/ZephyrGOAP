...
namespace ReGoap.Unity
{
    public class ReGoapPlannerThread<T, W>
    {
        ...
        private readonly Action<ReGoapPlannerThread<T, W>, ReGoapPlanWork<T, W>, IReGoapGoal<T, W>>
            onDonePlan;
    ...
    public class ReGoapPlannerManager<T, W> : MonoBehaviour
    {
        ...
        protected virtual void Awake()
        {
            ...
            {
                ReGoapLogger.Log(
                    $"[GoapPlannerManager] Running in multi-thread mode ({ThreadsCount} threads).");
            ...
            IReGoapGoal<T, W> newGoal)
        {
            ...
            if (work.NewGoal != null && ReGoapLogger.Level == ReGoapLogger.DebugLevel.Full)
            {
                ...
                {
                    ReGoapLogger.Log($"{i++}: {action.Action}");
                }
            }
        }
