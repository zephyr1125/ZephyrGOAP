...
namespace ReGoap.Planner
{
    public class ReGoapNode<T,W> : INode<ReGoapState<T,W>>
    {
        ...
        private void Init(IGoapPlanner<T, W> planner, ReGoapState<T, W> newGoal,
            ReGoapNode<T, W> parent, IReGoapAction<T, W> action, ReGoapState<T, W> settings)
        {
            expandList.Clear();
            ...
            var nextAction = parent?.action;
            if (action != null)
            {
                // create a new instance of the goal based on the parent goal
                Goal = ReGoapState<T, W>.Instantiate(newGoal);

                //打包给action计算用的参数组，其中settings为人工参数组
            ...
            Goal.MissingDifference(planner.GetCurrentAgent().GetMemory().GetWorldState(),
                ref diff);
        ...
        public List<INode<ReGoapState<T, W>>> Expand()
        {
            ...
            for (var index = actions.Count - 1; index >= 0; index--)
            {
                ...
                //猜测：一个action可以有多套setting，从而产生多个节点，比如mine(ore/stone)就是两个setting
