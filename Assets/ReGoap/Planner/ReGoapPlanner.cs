...
namespace ReGoap.Planner
{
    public class ReGoapPlanner<T,W>: IGoapPlanner<T,W>
    {
        ...
        public IReGoapGoal<T, W> Plan(IReGoapAgent<T, W> agent, 
            IReGoapGoal<T, W> blacklistGoal = null, 
            Queue<ReGoapActionState<T, W>> currentPlan = null, 
            Action<IReGoapGoal<T, W>> callback = null)
        {
            ...
            while (possibleGoals.Count > 0)
            {
                ...
                //检查是否goal里的所有state都能够被现有action的effect满足
                ...
                if (!settings.UsingDynamicActions)
                {
                    ...
                    foreach (var action in goapAgent.GetActionsSet())
                    {
                        ...
                        previous.MissingDifference(action.GetEffects(stackData),
                            ref wantedGoalCheck);
                    }
                    // finally push the current world state
                    var current = wantedGoalCheck;
                    wantedGoalCheck = ReGoapState<T, W>.Instantiate();
                    current.MissingDifference(GetCurrentAgent().GetMemory().GetWorldState(),
                        ref wantedGoalCheck);
                    // can't validate goal 
                ...
                var leaf = (ReGoapNode<T, W>)astar.Run(
                    ReGoapNode<T, W>.Instantiate(this, goalState, null,
                        null, null), goalState, settings.MaxIterations,
